...
Добавь в ответ 2 наиболее подходящих заголовка статей.
...
Запрос: "Муж может забрать мою посылку?"
Ответ:
{{
"samples": [
"Может ли супруг получить за меня посылку в отделении Почты России, и какие для этого нужны документы?",
"Узнать условия получения отправления доверенным лицом (супругом).",
"Порядок вручения. Заберет другой человек",
"Порядок вручения. Получение отправления членом семьи",
"другой человек может забрать мою посылку",
"какие документы нужны чтобы муж забрал посылку",
"можно ли получить чужую посылку"
]
}}

Запрос: "Не пришло извещение на письмо"
Ответ:
{
{
"samples": [
"Что делать, если не получено бумажное извещение на заказное письмо, и как узнать о его прибытии?",
"Получить письмо без извещения или узнать о его наличии.",
"Порядок вручения. Не пришло извещение на отправление",
"Порядок вручения. Получение письма без извещения",
"потерялось извещение на почтовое отправление",
"как узнать пришло ли письмо если нет извещения",
"поиск письма в отделении по адресу"
]
}}

Запрос: "Как получить по QR коду чужую посылку"
Ответ:
{{
"samples": [
"Как можно получить почтовое отправление по QR-коду, если я не являюсь адресатом? Какие нужны документы?",
"Получить отправление по QR-коду на имя другого человека.",
"QR-код. Получение по чужому QR-коду",
"Порядок вручения. Получение отправления по QR-коду третьим лицом",
"получить посылку по qr коду за другого",
"оформление доверенности для получения по qr",
"сканирование qr кода чужого отправления"
]
}}

Запрос: "Продлить до 27"
Ответ:
{{
"samples": [
"Как продлить срок хранения посылки до 27 числа текущего месяца в отделении Почты России?",
"Продлить хранение почтового отправления до конкретной даты.",
"Порядок вручения. Сроки хранения отправлений",
"Порядок вручения. Продление хранения почтовых отправлений",
"продлить хранение посылки до 27 числа",
"как продлить срок хранения отправления",
"увеличить срок хранения на почте"
]
}}

Запрос: "Что доставляют до двери"
Ответ:
{{
"samples": [
"Какие почтовые отправления доставляются курьером до двери получателя, а какие нужно забирать в отделении?",
"Узнать о критериях курьерской доставки отправлений на дом.",
"Порядок вручения. Что доставляют до двери",
"Порядок приёма. Услуги курьерской доставки",
"доставка посылок до двери почтой россии",
"курьерская доставка почтовых отправлений",
"какие отправления приносят на дом"
]
}}
`;
